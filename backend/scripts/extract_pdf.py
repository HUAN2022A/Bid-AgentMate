...
def extract_pdf_lines(input_path: str) -> tuple[list[str], dict]:
    """库函数入口：返回 (行列表, 统计信息)。单页失败不中断整体提取。"""
    lines = []
    n_tables = 0
    ...
    stats = {
        ...
        "tables": n_tables,
        "chars": total_chars,
        "maybe_scanned": bool(total_pages and total_chars / total_pages < 100),
    }
    return lines, stats
...
def main():
    ...
    lines, stats = extract_pdf_lines(args.input)
    ...
    print(
        f"页数: {stats['pages']}, 行数: {stats['lines']}, "
        f"表格: {stats['tables']} 个, 总字符: {stats['chars']} → {out}"
    )
